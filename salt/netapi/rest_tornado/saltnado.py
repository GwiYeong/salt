...
class SaltAPIHandler(BaseSaltAPIHandler, SaltClientsMixIn):  # pylint: disable=W0223
    ...
    def all_returns(self,
        ...
        for minion in minions_remaining:
            ...
            minion_event = self.application.event_listener.get_event(self,
                                                                     ...
                                                                     )  # apply timeout for getting minion data
        ...
        while True:
            try:
                ...
                if f is is_finished:
                    for event in minion_events.keys():
                        if not event.done():
                ...
                # Timeout future also returns True for the function done()
                pass

            # clear finished event future
            for done_event in [event for event in minion_events.keys() if event.done()]:
                # its possible to get a return that wasn't in the minion_remaining list
                try:
                    minions_remaining.remove(minion_events[done_event])
                except ValueError as e:
                    pass
                del minion_events[done_event]
    ...
    def job_not_running(self,
        ...
        while True:
            try:
                event_future = yield self.application.event_listener.get_event(self,
                                                                               tag=ping_tag,
                                                                               timeout=self.application.opts['gather_job_timeout'],
                                                                               )
                f = yield Any([event_future, is_finished])
                # When finished entire routine, cleanup other futures and return result
                if f is is_finished:
                    if not event.done():
                        event.set_result(None)
